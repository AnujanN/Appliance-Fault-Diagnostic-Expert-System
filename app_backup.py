...
input_mode = st.radio(
    ...
    ["🤖 Natural Language (AI-powered)", "📋 Manual Selection (Traditional)"],
...
if input_mode == "🤖 Natural Language (AI-powered)":

    st.markdown("#### 💬 Describe Your Problem")

    st.caption("Tell me what's wrong in your own words. Be as detailed as you like!")
    ...
    appliance_hint = st.selectbox(
        ...
        "Appliance type (optional - helps improve accuracy):",
    ...
    natural_language_text = st.text_area(
        ...
        placeholder="Example: My washing machine won't drain and it's making a grinding noise...",
    ...
    if natural_language_text.strip():

        st.info("💡 The AI will extract facts from your description automatically")



# --- Manual Selection Mode ---

elif input_mode == "📋 Manual Selection (Traditional)":
...
if st.button("Get Diagnosis", type="primary"):
    ...
    if input_mode == "🤖 Natural Language (AI-powered)":
        ...
        if not natural_language_text.strip():

            st.warning("⚠️ Please describe your problem in the text box above")
        ...
        with st.spinner("🤖 AI is analyzing your description..."):
            ...
            try:
                ...
                # Use appliance hint if not auto-detect

                hint = None if appliance_hint == "Auto-detect" else appliance_hint

                

                extracted_facts = extract_facts_from_text(natural_language_text, hint)

                

                if not extracted_facts:

                    st.error("❌ Could not extract facts from your description. Please try again or use Manual Selection mode.")
                ...
                with st.expander("🔍 What the AI understood from your description", expanded=True):

                    st.success("✅ Successfully extracted facts!")
                ...
                st.info("💡 Try using the Manual Selection mode instead")
    ...
    # Declare facts based on input mode
